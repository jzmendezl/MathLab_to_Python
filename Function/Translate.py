...
class Translate(matlabListener):
    ...
    def enterAssignment_statement(
            self, ctx: matlabParser.Assignment_statementContext):
        self.program += self.indentation()

    def enterPrimary_expression(self,
                                ctx: matlabParser.Primary_expressionContext):
        if ctx.IDENTIFIER():
            if ctx.IDENTIFIER().getText() in self.vars:
                self.program += self.vars[ctx.IDENTIFIER().getText()]
            else:
                if ctx.IDENTIFIER().getText() == "any":
                    self.program += self.indentation() + "np."
                self.program += ctx.IDENTIFIER().getText()
        if ctx.CONSTANT():
            if self.posArrayFlag:
                self.program += str(int(ctx.CONSTANT().getText()) - 1)
            else:
    ...
    def enterArray_expression(self, ctx: matlabParser.Array_expressionContext):
        if ctx.IDENTIFIER():
            if ctx.IDENTIFIER().getText() in self.vars:
                self.program += self.indentation() + self.vars[
    ...
    def enterIteration_statement(self,
                                 ctx: matlabParser.Iteration_statementContext):
        if ctx.WHILE():
            self.program += "\n"
    ...
    def exitIteration_statement(self,
                                ctx: matlabParser.Iteration_statementContext):
        if ctx.end():
    ...
    def enterIndex_expression_list(
            self, ctx: matlabParser.Index_expression_listContext):
        if ctx.comma():
